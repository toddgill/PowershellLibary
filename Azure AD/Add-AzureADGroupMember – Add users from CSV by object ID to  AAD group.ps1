...
#Purpose: Add listed users from a file to given AAD security group. Does not affect existing Members.
...
$groupIDs = 'Group Name' | Foreach-Object {Get-AzureADGroup -SearchString $_} | Select -ExpandProperty ObjectId; Import-Csv -path "C:\Temp\User-ObjectID-List.csv" | Foreach-Object {foreach($groupID in $groupIDs) {Add-AzureADGroupMember -ObjectId $groupID -RefObjectId $_.ObjectId}}
